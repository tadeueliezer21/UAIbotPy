...
box = Box(htm = np.identity(4), width=0.4, depth=0.4, height=0.5, mesh_material=material_box)
...
sim.add([box,light1,light2,light3,light4, explanation, vector_v, vector_w, frame])

dt=0.01
htm = box.htm

fun = lambda t: Utils.trn([0,0,0.6]) @ Utils.rotz(2*t)
fun2 = lambda t: fun(t) @ Utils.trn([-0.2,0,0])

for i in range(3000):
    htm =  fun(i*dt)
    d_htm_dt = (fun2((i+1)*dt)-fun2((i-1)*dt))/(2*dt)
    Q = htm[0:3,0:3]
    ...
    frame.add_ani_frame(i*dt,htm=htm @ Utils.trn([-0.2,0,0]))
    htm2 = fun2(i*dt)
    vector_v.add_ani_frame(i * dt, origin=htm2[0:3,-1], vector=0.4*v)
    vector_w.add_ani_frame(i * dt, origin=htm2[0:3,-1], vector=0.4*w)
...
sim.save("D:\\PycharmProjects\\aulas_manipuladores\\presentation\\images\\aula4","anim4")
